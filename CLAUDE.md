...
- **Decorative layers must have `pointer-events:none`.** An element with `opacity<1` creates a stacking context that paints above later in-flow siblings. The footer `.big-name` overflowed onto the social links this way and swallowed their clicks. To verify, use `document.elementFromPoint` on link centres.
